...
describe("DExPair", () => {
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      ).to.be.revertedWithCustomError(pair, "Operation_invalid");
  ...
  const optimisticTestCases = [
    ...
    a.map((n) => (typeof n === "string" ? bigNumberify(n) : toWei(n)))
  );
  ...
  it("swap:token0", async () => {
    ...
    const expectedOutputAmount = getAmount(
      swapAmount,
      reserves[0],
      reserves[1]
    ); // 1662497915624478906
  ...
  it("price{0,1}CumulativeLast", async () => {
    ...
    expect(await pair.LastPrice_A()).to.eq(initialPrice[0]);
    expect(await pair.LastPrice_B()).to.eq(initialPrice[1]);
    ...
    expect(await pair.LastPrice_A()).to.eq(initialPrice[0].mul(10)); // +10 blocks since the beginning
    expect(await pair.LastPrice_B()).to.eq(initialPrice[1].mul(10));
    ...
    expect(await pair.LastPrice_A()).to.eq(
      initialPrice[0].mul(10).add(newPrice[0].mul(10))
    );
    expect(await pair.LastPrice_B()).to.eq(
