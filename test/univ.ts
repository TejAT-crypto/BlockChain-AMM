...
export const sortTokensAddresses = (...adddresses: string[]) =>
  [...adddresses].sort((t1, t2) => (t1 < t2 ? -1 : 1));
export const sortTokens = (...tokens: ERC20[]) =>
  [...tokens].sort((t1, t2) => (t1.address < t2.address ? -1 : 1));
...
export const encodePrice = (reserve0: BigNumberish, reserve1: BigNumberish) => {
  ...
  return [
    reserve1BN.mul(TWO_POW112).div(reserve0BN),
    reserve0BN.mul(TWO_POW112).div(reserve1BN),
  ];
};
...
export const getAmount = (
  ...
  return inputAmountLessFees
    .mul(outputReserveBN)
    .div(inputReserveBN.add(inputAmountLessFees));
};
