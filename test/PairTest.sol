...
contract PairTest is Test{
    ...
    ERC20token token1;
    DExPair pair;
    MockProvider testuser;

     function setUp() public {
        testUser = new MockProvider ();

        token0 = new ERC20token("Token A", "TKNA");
        token1 = new ERC20token("Token B", "TKNB");

        factory fact = new factory();
        address pairAddress = fact.createPairs(
            address(token0),
            address(token1)
        );
...
contract TestUser {
    function provideLiquidity(
        ...
        DExPair(pairAddress_).mint(address(this));
    }
    ...
    function removeLiquidity(address pairAddress_) public {
        ...
        DExPair(pairAddress_).burn(address(this));
    }
}
