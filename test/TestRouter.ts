...
describe('Router', () => {
  const deployFixture = async () => {
    ...
    return { Factory, pair, wethPair, token0, token1, weth, wethPartner, RouterTest, owner, other };
  };
  ...
  it('addLiquidity', async () => {
    const { token0, token1, RouterTest: router, pair, owner } = await loadFixture(deployFixture);
  ...
  it('addLiquidityETH', async () => {
    const { wethPartner, wethPair, owner, RouterTest: router } = await loadFixture(deployFixture);
  ...
  it('removeLiquidity', async () => {
    const { token0, token1, pair, owner, RouterTest: router } = await loadFixture(deployFixture);
    const token0Amount = toWei(1);
    const token1Amount = toWei(4);
